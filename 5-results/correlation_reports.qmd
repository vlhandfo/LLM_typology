...
```{python}
show(delta_corr, classes="compact", paging=False)
```

The features with correlations with delta and pvalue < 0.01. There is a different threshold for acceptance here to limit the number of correlations because there were 56 task/feature pairs with pvalues < 0.05.
:::

#### Low variance

In this case, there were no features with low variance.
